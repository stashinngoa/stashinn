...
import { useState, FormEvent, useEffect, useTransition as reactUseTransition } from 'react';
...
export default function SearchHeader({ initialSearch }: { initialSearch: any }) {
  ...
  return (
    <form onSubmit={handleSearch} className="flex-1 max-w-4xl hidden md:flex items-center bg-gray-50 rounded-full border border-gray-200 px-2 py-1 shadow-sm hover:shadow transition-shadow relative">
      ...
      <button type="submit" disabled={isPending} className={`w-10 h-10 ml-2 rounded-full text-white flex items-center justify-center transition-transform shrink-0 ${isPending ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-purple-600 to-indigo-600 hover:scale-105'}`}>
        {isPending ? (
